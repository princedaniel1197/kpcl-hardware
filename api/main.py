...
The WebSocket carries live events relayed by the collector over Postgres
LISTEN/NOTIFY. The REST endpoints read the archive.
...
async def tags() -> list[dict]:
    rows = await fetch(
        ...
        " t.range_high, t.source_system, e.asset_code"
...
async def trend(tag_name: str, minutes: float = Query(10, gt=0, le=1440),
    ...
    if not rows:
        meta = await fetch("SELECT 1 FROM tag WHERE name = %s", (tag_name,))
        if not meta:
            raise HTTPException(404, f"no such tag: {tag_name}")
    points = [{"source_ts": r[0].isoformat(), "server_ts": r[1].isoformat(),
...
async def assets() -> list[dict]:
    ...
    return [{"id": r[0], "asset_code": r[1], "name": r[2], "level": r[3],
             "parent_id": r[4], "template": r[5]} for r in rows]
...
async def kpis(asset_code: str | None = None) -> list[dict]:
    ...
    return [{"kpi": r[0], "classification": r[1], "version": r[2],
             ...
             "validity_low": r[11], "validity_high": r[12]} for r in rows]
...
async def replay(start: str, end: str,
    ...
    for name, source_ts, server_ts, value, quality, klass in rows:
        series[name].append({
            ...
            "server_ts": server_ts.isoformat(), "value": value,
